...
export const home = {
  en: {
    nav: ['Projects', 'Hackathons', 'About'], contact: 'Say hello',
    eyebrow: 'Arnaud Durand · Paris, France', title: 'Curious by nature.', titleEnd: 'Building, together.',
    intro: "I build AI tools, with a soft spot for the systems underneath. From a UNIX shell to a voice journal, I like turning an idea into something we can try, question and improve.",
    ...
    projects: projects.filter(p=>p.story).map(p=>({name:p.name,category:p.tags.join(' · '),text:p.en.summary,role:p.en.role,url:p.repo,id:p.id})),
    ...
    systems:[{name:'Minishell',text:'A shell in C · pipes, redirections & processes'},{name:'Inception',text:'Docker infrastructure · NGINX, WordPress & MariaDB'},{name:'Philosophers',text:'Concurrency in C · threads & mutexes'}],
    ...
    path:'Along the way', experiences:[['2026','BPCE','AI platform internship · internal proof of concept'],['2025–2026','BNP Paribas','Data Scientist internship · document analysis & LLM observability'],['2019–2024','École 42 Paris','Learning through software projects']],
    endTitle:'Something worth building?', endText:'I’m looking for a permanent role in applied AI or AI platforms. I’m also happy to talk about your project, a hackathon or an idea. French native; English fluent; Spanish B1.', mail:'Let’s talk', footer:'Made with curiosity, in Paris.', skip:'Skip to content'
  },
  fr: {
    nav:['Projets','Hackathons','À propos'], contact:'Échangeons',
    eyebrow:'Arnaud Durand · Paris, France', title:'Curieux, naturellement.', titleEnd:'Construire, ensemble.',
    intro:'Je construis des outils IA, avec un faible pour les systèmes qui les font tourner. D’un shell UNIX à un journal vocal, j’aime donner forme à une idée pour pouvoir l’essayer, la questionner et l’améliorer.',
    ...
    projects: projects.filter(p=>p.story).map(p=>({name:p.name,category:p.tags.join(' · '),text:p.fr.summary,role:p.fr.role,url:p.repo,id:p.id})),
    ...
    systems:[{name:'Minishell',text:'Un shell en C · pipes, redirections et processus'},{name:'Inception',text:'Infrastructure Docker · NGINX, WordPress et MariaDB'},{name:'Philosophers',text:'Concurrence en C · threads et mutex'}],
    ...
    path:'Au fil du parcours',experiences:[['2026','BPCE','Stage plateforme IA · preuve de concept interne'],['2025–2026','BNP Paribas','Stage Data Scientist · analyse documentaire et observabilité LLM'],['2019–2024','École 42 Paris','Apprendre par les projets logiciels']],
    endTitle:'Une idée à construire ?',endText:'Je cherche un CDI en IA appliquée ou plateforme IA. Je serais aussi ravi d’échanger sur ton projet, un hackathon ou une idée. Français langue maternelle ; anglais courant ; espagnol B1.',mail:'Parlons-en',footer:'Fait avec curiosité, à Paris.',skip:'Aller au contenu'
  }
};
