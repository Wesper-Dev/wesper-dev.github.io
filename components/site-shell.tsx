...
export function Header({locale, path = ''}:{locale:Locale;path?:string}) {
 ...
 return <><a className="skip" href="#main">{t.skip}</a><header className="header wrap"><a className="wordmark" href={pagePath(locale)} aria-label={fr?'Arnaud Durand — accueil':'Arnaud Durand — home'}>ad<span>.</span></a><nav aria-label={fr?'Navigation principale':'Main navigation'}>{t.nav.map((name,i)=><a key={name} href={pagePath(locale,['projects/','hackathons/','skills/','#about'][i])} aria-current={(i===0&&path.startsWith('projects/'))||(i===1&&path==='hackathons/')||(i===2&&path==='skills/')?'page':undefined}>{name}</a>)}<a className="language" href={pagePath(fr?'en':'fr',path)} hrefLang={fr?'en':'fr'} lang={fr?'en':'fr'} aria-label={fr?'Read in English':'Lire en français'}>{fr?'EN':'FR'}</a></nav></header></>;
}
